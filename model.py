...
class Generator(nn.Module):
    def __init__(self,input_dim: int=3,trunk_a_count : int=8,trunk_rfb_count: int=8) -> None:
        super(Generator, self).__init__()
        # The first layer of convolutional layer.
        self.conv1 = nn.Conv2d(input_dim, 64, (3, 3), (1, 1), (1, 1))

        # Trunk-a backbone network.
        trunk_a = []
        for _ in range(trunk_a_count):
            trunk_a.append(ResidualInResidualDenseBlock(64, 32))
        self.trunk_a = nn.Sequential(*trunk_a)

        # Trunk-RFB backbone network.
        trunk_rfb = []
        for _ in range(trunk_rfb_count):
